...
RUN apk add --no-cache --update \
    ca-certificates nginx dcron curl tini \
    php7 php7-bcmath php7-common php7-dom php7-fpm php7-gd php7-mbstring \
    php7-openssl php7-zip php7-pdo php7-phar php7-json php7-pdo_mysql \
    php7-session php7-ctype php7-tokenizer php7-zlib php7-simplexml supervisor \
    && curl -sS https://getcomposer.org/installer | php -- --install-dir=/usr/local/bin --filename=composer

# Télécharger le panel Pterodactyl
RUN curl -Lo panel.tar.gz https://github.com/pterodactyl/panel/releases/download/v0.7.10/panel.tar.gz \
    && tar --strip-components=1 -xzvf panel.tar.gz \
    && chmod -R 755 storage/* bootstrap/cache/

# Ne PAS créer un .env temporaire, laisse Render injecter les variables
